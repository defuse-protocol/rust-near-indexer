use std::collections::HashMap;

use crate::types::{EventRow, SilverDip4TransferRow};

/// Production accounts for silver_dip4_transfer (same filter as the ClickHouse MV).
const PRODUCTION_CONTRACT_IDS: &[&str] = &["defuse-alpha.near", "intents.near"];

/// Extract denormalized silver DIP-4 transfer rows from raw event rows.
///
/// Mirrors the ClickHouse materialized view `mv_silver_dip4_transfer`:
/// 1. Build a referral lookup from `dip4` / `token_diff` events.
/// 2. Parse `dip4` / `transfer` events, flatten `tokens` map, join referral.
/// 3. Filter to production contract IDs only.
pub fn extract_silver_dip4_transfers(events: &[EventRow]) -> Vec<SilverDip4TransferRow> {
    // Step 1: Build referral lookup keyed by related_receipt_id.
    // A token_diff event's data array may contain items with a `referral` field.
    // We take the first non-null referral per receipt.
    let mut referral_by_receipt: HashMap<&str, String> = HashMap::new();
    for ev in events {
        if ev.standard != "dip4" || ev.event != "token_diff" {
            continue;
        }
        if !PRODUCTION_CONTRACT_IDS.contains(&ev.contract_id.as_str()) {
            continue;
        }
        if referral_by_receipt.contains_key(ev.related_receipt_id.as_str()) {
            continue;
        }
        if let Ok(arr) = serde_json::from_str::<Vec<serde_json::Value>>(&ev.data) {
            for item in &arr {
                if let Some(referral) = item.get("referral").and_then(|v| v.as_str())
                    && !referral.is_empty()
                {
                    referral_by_receipt.insert(&ev.related_receipt_id, referral.to_string());
                    break;
                }
            }
        }
    }

    // Step 2: Parse transfer events and flatten tokens map.
    let mut rows = Vec::new();
    for ev in events {
        if ev.standard != "dip4" || ev.event != "transfer" {
            continue;
        }
        if !PRODUCTION_CONTRACT_IDS.contains(&ev.contract_id.as_str()) {
            continue;
        }

        let data_arr: Vec<serde_json::Value> = match serde_json::from_str(&ev.data) {
            Ok(arr) => arr,
            Err(_) => continue,
        };

        let referral = referral_by_receipt
            .get(ev.related_receipt_id.as_str())
            .cloned();

        for item in &data_arr {
            let memo = item
                .get("memo")
                .and_then(|v| v.as_str())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string());
            let old_owner_id = item
                .get("account_id")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let new_owner_id = item
                .get("receiver_id")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let intent_hash = item
                .get("intent_hash")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string();

            // tokens is a map: { "token_id": "amount_string", ... }
            let tokens = match item.get("tokens").and_then(|v| v.as_object()) {
                Some(map) => map,
                None => continue,
            };

            for (token_id, amount_val) in tokens {
                let amount = match amount_val.as_str() {
                    Some(s) => s.to_string(),
                    None => amount_val.to_string().trim_matches('"').to_string(),
                };

                rows.push(SilverDip4TransferRow {
                    block_height: ev.block_height,
                    block_timestamp: ev.block_timestamp,
                    block_hash: ev.block_hash.clone(),
                    tx_hash: ev.tx_hash.clone().unwrap_or_default(),
                    contract_id: ev.contract_id.clone(),
                    execution_status: ev.execution_status.clone(),
                    version: ev.version.clone(),
                    standard: ev.standard.clone(),
                    event: ev.event.clone(),
                    related_receipt_id: ev.related_receipt_id.clone(),
                    related_receipt_receiver_id: ev.related_receipt_receiver_id.clone(),
                    related_receipt_predecessor_id: ev.related_receipt_predecessor_id.clone(),
                    memo: memo.clone(),
                    old_owner_id: old_owner_id.clone(),
                    new_owner_id: new_owner_id.clone(),
                    token_id: token_id.clone(),
                    amount,
                    intent_hash: intent_hash.clone(),
                    referral: referral.clone(),
                });
            }
        }
    }

    rows
}
