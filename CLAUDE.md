...
- **BlocksAPI producer reconnect**: both binaries wrap the `blocksapi::streamer` build + `tokio::select!` in a `loop` that resumes from `metrics::LATEST_BLOCK_HEIGHT.get()` (a `prometheus::IntGauge`, internally atomic, already set once per successful block in `handle_streamer_message`). On producer error (`h2`/`tonic` stream cut) the loop rebuilds the streamer from `latest + 1` after a 2s backoff. The cursor is **in-RAM only** — never re-query `get_last_height_*` on reconnect, because a reindexer and the live indexer share the destination DB at very different heights and DB max() would drag the reindexer to the tip. The metric must continue to have exactly one writer (`handle_streamer_message`); do not `set()` it from anywhere else or the cursor breaks. Consumer errors (DB insert failures, extractor panics) still propagate immediately and kill the process — those are real bugs, not transient.
